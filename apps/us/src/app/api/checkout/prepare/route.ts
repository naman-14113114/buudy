...
const freeGiftCode = "free_bundle_us";
const PLUSBASE_PRODUCTS: Record<string, { productId: number; variantId: number }> = {
  "buudy-led-mask": { productId: 1000000667824913, variantId: 1000020463775916 },
  "buudy-red-torch": { productId: 1000000667833408, variantId: 1000020464156040 },
};
...
function buildAttributionProperties(attribution: CheckoutPrepareBody["attribution"]) {
  return Object.entries(cleanAttribution(attribution)).map(([key, value]) => ({
    name: `_blfm_${key}`,
    value,
  }));
}
  ...
  if (cart?.lines?.length) {
    for (const line of cart.lines) {
      ...
      await addItem(
        ...
        line.quantity,
        buildAttributionProperties(attribution),
      );
