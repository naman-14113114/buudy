...
async function createPlusbaseCheckout(
  ...
  let cookie = "X-Global-Market=US; X-Global-Market-Currency=USD; X-Lang=en-us; country_code=US; currency=USD";
...
export async function POST(request: NextRequest) {
  ...
  if (
    countryHeader?.trim().toUpperCase() === "MA" ||
    countryHeader?.trim().toUpperCase() === "MOROCCO"
  ) {
