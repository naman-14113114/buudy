...
const detailIcons = [Building2, Mail, Phone, Building2];
      ...
      <div className="buudy-wrap relative z-10 grid gap-12 lg:grid-cols-[1.02fr_.98fr] lg:items-center">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <span className="buudy-mono rounded-full bg-[var(--plum)] px-4 py-3 text-[var(--cream)]">
...
function ContactDetails() {
  return (
    <section className="buudy-section bg-[var(--blush)] py-20">
      <div className="buudy-wrap">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {contactDetails.map((item, index) => {
            const Icon = detailIcons[index] ?? Building2;

            return (
              <Link
                className="group rounded-[18px] border border-[var(--border)] bg-[var(--card)] p-5 transition hover:-translate-y-1 hover:shadow-[0_22px_46px_-36px_rgba(58,31,61,.5)]"
                href={item.href}
                key={item.label}
              >
                <span className="grid h-10 w-10 place-items-center rounded-full bg-[rgba(58,31,61,.07)] text-[var(--plum)] transition group-hover:bg-[var(--plum)] group-hover:text-[var(--cream)]">
                  <Icon size={18} />
                </span>
                <p className="buudy-mono mt-5 text-[var(--gold)]">{item.label}</p>
                <p className="mt-3 text-sm leading-6 text-[var(--plum)]">{item.value}</p>
              </Link>
            );
          })}
        </div>
